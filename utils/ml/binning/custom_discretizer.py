...
class CustomDiscretizer(_BaseDiscretizer):
    ...
    * fit & transform with `pandas`
        ...
        - inplace discretization
            >>> import pandas as pd
            >>> import numpy as np
            >>> from ml.binning import CustomDiscretizer
    ...
    * fit & transform with `koalas`
    ...
    >>> from ml.binning import CustomDiscretizer
    >>> bins = {'A':[-np.inf, 0, np.inf], 'B':[-np.inf, 1, np.inf]}
    >>> X = ps.DataFrame({'A': [-1, 0, 1], 'B': [1, 2, 3]})
    ...
    * fit with `koalas` & transform with `NumPy`
    ...
    See Also
    --------
    ml.binning.Discretizer
        Discretize using equal splits.
    ml.binning.QuantileDiscretizer
    ...
    def fit(self, X: Union[pd.DataFrame, ps.DataFrame], y=None) -> "CustomDiscretizer":
