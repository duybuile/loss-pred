...
class BinRareEvents(Transformer):
    ...
    >>> import pandas as pd
    >>> from ml.binning import BinRareEvents
    ...
    * fit with `koalas` & transform with `NumPy`
    ...
    def fit(self, X: Union[pd.DataFrame, ps.DataFrame], y=None) -> "BinRareEvents":
        ...
        self.n_categories_to_keep_np = self.categories_to_keep_np.shape[0] - (self.categories_to_keep_np is None).sum(0)
    ...
    def compute_categories_to_keep_dict(
        X: Union[pd.DataFrame, ps.DataFrame], min_ratio: float
    ) -> Dict[str, List[str]]:
