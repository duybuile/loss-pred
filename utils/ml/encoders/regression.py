...
from ...df.transformer import Transformer
from ._base_encoder import _BaseEncoder
from .multiclass import MultiClassEncoder
...
class RegressionEncoder(_BaseEncoder):
    ...
    * fit & transform with `pandas`
    ...
    >>> import pandas as pd
    >>> from ml.binning import QuantileDiscretizer
    >>> from ml.encoders import WOEEncoder
    >>> from ml.encoders import RegressionEncoder
    >>> X = pd.DataFrame({
    ... 'A': ['Q', 'Q', 'Q', 'W', 'W', 'W'],
    ... 'B': ['Q', 'Q', 'W', 'W', 'W', 'W'],
    ... 'C': ['Q', 'Q', 'Q', 'Q', 'W', 'W'],
    ...
    * fit & transform with `koalas`
    ...
    ... 'C': ['Q', 'Q', 'Q', 'Q', 'W', 'W'],
    ... 'D': [1, 2, 3, 4, 5, 6]})
    >>> y = ps.Series([0.11,  -0.1, 5.55, 233.9, 4.66, 255.1], name='TARGET')
    >>> obj = RegressionEncoder(
    ... encoder=WOEEncoder(),
    ... discretizer=QuantileDiscretizer(n_bins=3, inplace=True))
    >>> obj.fit_transform(X, y)
    ...
    * fit with `koalas` & transform with `NumPy`
    ...
    def fit(
        self, X: Union[pd.DataFrame, ps.DataFrame], y: Union[pd.Series, ps.Series]
    ) -> "RegressionEncoder":
        ...
        X : Union[pd.DataFrame, ps.DataFrame].
            Input dataframe.
        y : Union[pd.Series, ps.Series], default to None.
            Labels.
