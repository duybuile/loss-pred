...
class TargetEncoder(_BaseEncoder):
    ...
    * fit & transform with `pandas`
    ...
    >>> import pandas as pd
    >>> from ml.encoders import TargetEncoder
    ...
    * fit with `koalas` & transform with `NumPy`
    ...
    def generate_mapping(
        ...
        for name in X.columns:
            dummy = (
                ps.DataFrame(X[name]).join(y).groupby(name).mean()[y_name].to_pandas()
            )
            dummy.name = name
