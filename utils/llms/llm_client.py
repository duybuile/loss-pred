...
class LLMClient:
    ...
    def call(self, prompt: Optional[str] = "", log_usage: bool = False, **kwargs_additional) -> str | None:
        ...
        Parameters
        ----------
        prompt: optional prompt to send to the LLM. When it is None or "", input must be provided
        ...
        if self.api == "openai":
            ...
            if not kwargs_additional.get("input"):
                # Use a simple user input with prompt when input is not there
                if prompt == "":
                    logger.warning("Prompt is empty and input is not there")
                    return None
                kwargs["input"] = prompt
